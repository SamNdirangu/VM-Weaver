...
apk_installed() { apk info -e "$1" &>/dev/null; }
...
install_deps() {
    ...
    # Alpine package names differ from Ubuntu
...
list_disk_files() {
    ...
    for f in "${FILES[@]}"; do
        ...
        local fmod;  fmod=$(stat -c '%y' "$f" 2>/dev/null | cut -d' ' -f1,2 | cut -c1-16 \
            || echo "unknown")
...
write_to_disk() {
    ...
    local DISKS=()
    # Alpine busybox lsblk may not support -o TRAN; use a safe minimal column set
